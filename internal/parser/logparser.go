// Package parser provides utilities for parsing Renovate JSON log output.
package parser
...
func ParseRenovateLogs(logs string) *LogParseResult {
	...
	for scanner.Scan() {
		...
		case entry.Msg == "Repository finished":
			handleRepositoryFinished(line, result)
		case entry.Msg == "Creating PR":
			handleCreatingPR(line, branchMap)
		case entry.Msg == "Updating PR":
			handleUpdatingPR(line, branchMap)
		case strings.Contains(entry.Msg, "does not need updating"):
			handleDoesNotNeedUpdating(line, entry.Msg, branchMap)
		case entry.Msg == "git push":
			handleGitPush(line, branchMap)
		case entry.Msg == "PR created":
			handlePRCreated(line, branchMap)
		case entry.Msg == "PR automerged":
			handlePRAutomerged(line, branchMap)
		case entry.Msg == "branches info extended":
			handleBranchesInfoExtended(line, branchMap)
		}
	}
...
// handleRepositoryFinished parses the "Repository finished" log entry and sets the result status.
func handleRepositoryFinished(line string, result *LogParseResult) {
...
// handleCreatingPR parses the "Creating PR" log entry and records the branch action.
func handleCreatingPR(line string, branchMap map[string]*api.PRDetail) {
...
// handleUpdatingPR parses the "Updating PR" log entry and records the branch action.
func handleUpdatingPR(line string, branchMap map[string]*api.PRDetail) {
	var pr prCreateUpdateEntry
	if err := json.Unmarshal([]byte(line), &pr); err == nil && pr.Branch != "" {
		detail := getOrCreateDetail(branchMap, pr.Branch)
...
// handleDoesNotNeedUpdating parses messages containing "does not need updating" and records unchanged PRs.
func handleDoesNotNeedUpdating(line, msg string, branchMap map[string]*api.PRDetail) {
...
// handleGitPush parses the "git push" log entry and extracts PR numbers from remote messages.
func handleGitPush(line string, branchMap map[string]*api.PRDetail) {
	var gp gitPushEntry
...
// handlePRCreated parses the "PR created" log entry and records the PR number.
func handlePRCreated(line string, branchMap map[string]*api.PRDetail) {
...
// handlePRAutomerged parses the "PR automerged" log entry and records the automerge action.
func handlePRAutomerged(line string, branchMap map[string]*api.PRDetail) {
	var pc prCreatedEntry
...
// handleBranchesInfoExtended parses the "branches info extended" log entry and backfills branch data.
func handleBranchesInfoExtended(line string, branchMap map[string]*api.PRDetail) {
